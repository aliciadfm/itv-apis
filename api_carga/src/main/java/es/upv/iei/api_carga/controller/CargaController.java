...
public class CargaController {
    ...
    @Operation(
            summary = "Ejecutar proceso de carga desde fuentes externas",
            description = """
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
                    description = "Proceso de carga ejecutado correctamente",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = ResultadoCargaDTO.class)
                    )
            ),
            @ApiResponse(
                    responseCode = "400",
                    description = "Fuente no reconocida o parámetros inválidos",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(
                                    example = """
            ...
            @ApiResponse(
                    responseCode = "500",
                    description = "Error interno durante el proceso ETL",
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(
                                    example = """
    ...
    public ResponseEntity<ResultadoCargaDTO> cargar(
            ...
            @RequestParam List<String> fuentes
    ) {
        ResultadoCargaDTO resultado = cargaService.cargarDesdeFuentes(fuentes);
        return ResponseEntity.ok(resultado);
    }

    @Operation(
            summary = "Eliminar todos los datos cargados",
            description = """
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    ...
                    content = @Content(
                            ...
                            schema = @Schema(example = "Datos eliminados correctamente")
                    )
            ),
            @ApiResponse(
                    responseCode = "500",
                    description = "Error interno durante la eliminación de datos"
            )
    })
